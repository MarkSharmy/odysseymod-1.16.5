...
public abstract class VerticalRedstoneDiodeBlock extends BaseBlock {
	...
	protected int calculateInputStrength(World worldIn, BlockPos pos, BlockState state) {
		...
		if (i >= 15) {
			...
			return Math.max(i, blockstate.isIn((ITag<Block>) Blocks.REDSTONE_WIRE) ? blockstate.get(RedstoneWireBlock.POWER) : 0);
		}
	}
	...
	protected int getPowerOnSide(IWorldReader worldIn, BlockPos pos, Direction side) {
		...
		if (this.isAlternateInput(blockstate)) {
			if (blockstate.isIn((ITag<Block>) Blocks.REDSTONE_BLOCK))
				return 15;
			else
				return blockstate.isIn((ITag<Block>) Blocks.REDSTONE_WIRE) ? blockstate.get(RedstoneWireBlock.POWER) : worldIn.getStrongPower(pos, side);
		} else
			return 0;
	}
