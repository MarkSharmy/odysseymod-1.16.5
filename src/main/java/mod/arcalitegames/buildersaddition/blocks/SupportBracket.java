...
public class SupportBracket extends BaseDerivativeBlock {
	...
	public SupportBracket(String name) {
		...
		setDefaultState(getDefaultState().with(FACING, Direction.NORTH).with(SUPPORTING, Direction.UP).with(MOUNT, MountState.FULL));
	}
	...
	private VoxelShape getShapeNorth(MountState s, boolean up) {
		switch(s) {
			case FULL: return up ? SHAPE_UP_NORTH : SHAPE_DOWN_NORTH;
	...
	private VoxelShape getShapeSouth(MountState s, boolean up) {
		switch(s) {
			case FULL: return up ? SHAPE_UP_SOUTH : SHAPE_DOWN_SOUTH;
	...
	private VoxelShape getShapeEast(MountState s, boolean up) {
		switch(s) {
			case FULL: return up ? SHAPE_UP_EAST : SHAPE_DOWN_EAST;
	...
	private VoxelShape getShapeWest(MountState s, boolean up) {
		switch(s) {
			case FULL: return up ? SHAPE_UP_WEST : SHAPE_DOWN_WEST;
