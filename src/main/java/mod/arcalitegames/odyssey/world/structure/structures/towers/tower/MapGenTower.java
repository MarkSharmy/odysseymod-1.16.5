...
public class MapGenTower extends MapGenStructure implements Serializable
{
	
	/**
	 * 
	 */
	private static final long serialVersionUID = 6509390908204808842L;

	@Override
	public boolean canGenerate(BlockPos start)
    {
    	
    	StructureClass structureClass = Structures.TOWER.getStructureClass();
    ...
    public static class StructurePiece
    {
        ...
        public void generate(WorldTickEvent event)
        {
            if(origin != null)
            {
                structure = new Tower(event.world, origin, 0, 0, 0);
                generatePart = 1;
                origin = null;
            }
        }
