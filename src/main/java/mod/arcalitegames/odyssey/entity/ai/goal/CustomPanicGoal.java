...
public class CustomPanicGoal extends Goal {
   ...
   public CustomPanicGoal(CitizenEntity citizen)
   {
	   this(citizen, 0.5);
   }
   
   public CustomPanicGoal(CitizenEntity citizen, double speedIn) {
   ...
   public boolean shouldExecute() {
      if (this.citizen.getRevengeTarget() == null && !this.citizen.isBurning()) {
         return false;
      } else {
         if (this.citizen.isBurning()) {
            BlockPos blockpos = this.getRandPos(this.citizen.world, this.citizen, 5, 4);
            if (blockpos != null) {
   ...
   protected boolean findRandomPosition() {
      Vector3d vector3d = RandomPositionGenerator.findRandomTarget(this.citizen, 5, 4);
      if (vector3d == null) {
         return false;
      } else {
   ...
   public void startExecuting() {
      this.citizen.getNavigator().tryMoveToXYZ(this.randPosX, this.randPosY, this.randPosZ, this.speed);
